...
export function FilterSidebar({ facets, filters }:
  ...
  return (
    <>
      {/* ≥ md: sticky sidebar */}
      <aside className="sticky top-6 hidden max-h-[calc(100vh-3rem)] w-44 flex-none overflow-auto rounded-2xl border border-line bg-surface/60 p-4 backdrop-blur-sm md:block md:w-52">
      ...
      <button
        ...
        className="fixed bottom-5 left-1/2 z-40 inline-flex -translate-x-1/2 items-center gap-2 rounded-full border border-line-2 bg-surface/90 px-5 py-2.5 text-sm text-ink shadow-[0_12px_34px_-12px_rgba(0,0,0,0.9)] backdrop-blur-md md:hidden"
      >
        ...
        絞り込み
        {activeCount > 0 && (
          <span className="rounded-full bg-shu px-1.5 text-[10px] font-bold leading-[16px] text-[#1b0c08]">{activeCount}</span>
        )}
      </button>
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true" aria-label="絞り込み">
          <div className="drawer-backdrop absolute inset-0 bg-black/65 backdrop-blur-sm" onClick={() => setOpen(false)} />
          <div className="drawer-panel absolute inset-y-0 left-0 flex w-[84%] max-w-xs flex-col border-r border-line bg-surface">
            <div className="flex items-center justify-between border-b border-line px-4 py-3">
              <span className="text-[11px] uppercase tracking-[0.22em] text-muted">絞り込み · Filter</span>
            ...
            <div className="border-t border-line p-3">
              <button type="button" onClick={() => setOpen(false)}
                className="w-full rounded-full bg-shu py-2.5 text-sm font-medium text-[#1b0c08]">
                結果を見る
              </button>
            </div>
          </div>
