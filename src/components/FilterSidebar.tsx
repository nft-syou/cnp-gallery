  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-5 left-1/2 z-40 inline-flex -translate-x-1/2 items-center gap-2 rounded-full bg-cnp px-5 py-2.5 text-sm font-bold text-ink shadow-[0_14px_34px_-10px_rgba(255,202,0,0.95)] md:hidden"
      >
        ...
        絞り込み
        {activeCount > 0 && (
          <span className="rounded-full bg-ink px-1.5 text-[10px] font-bold leading-[16px] text-cnp">{activeCount}</span>
        )}
      </button>
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true" aria-label="絞り込み">
          <div className="drawer-backdrop absolute inset-0 bg-ink/40 backdrop-blur-sm" onClick={() => setOpen(false)} />
          <div className="drawer-panel absolute inset-y-0 left-0 flex w-[84%] max-w-xs flex-col border-r border-line bg-surface">
            ...
            <div className="border-t border-line p-3">
              <button type="button" onClick={() => setOpen(false)}
                className="w-full rounded-full bg-cnp py-2.5 text-sm font-bold text-ink transition hover:bg-cnp-deep">
                結果を見る
              </button>
            </div>
          </div>
